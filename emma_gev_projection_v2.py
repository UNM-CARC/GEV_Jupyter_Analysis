...
for workload in workloads:
    ...
    for num in range(len(df_platform_new)):
        fname = 'Emma/'+platform+'_'+workload+'_256_'+str(num)+'.npy'
        print(fname)
        ...
        for i in range(_NNRANKS):
            # select a sample of the workload of size _NNODES
            s1=sample_data[0:_SAMPLE_NNODES,i,0:_NITER]
    ...
    plt.title(platform+' '+workload+' -  per node bootstrap, global CIs')
    plt.tight_layout()
    #plt.show()
    fig.savefig('Emma/figs/'+platform+'_'+workload+'_v2.png')
    plt.close(fig)
    fn=fn+1
